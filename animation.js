...
function animateJS(CL) {
    ...
    } else if (nameCounter <= 65) { // \tpageViews
        document.getElementById('titlestruct3').innerText = jw.slice(55, nameCounter);
    } else if (nameCounter == 66) { // :
        document.getElementById('titlestruct3').style.color = '#9cdcfe';
        document.getElementById('titlecolon3').innerText = jw.slice(65, nameCounter);
    } else if (nameCounter <= 67 + CL) { // _{CLICK_Count}
        document.getElementById('titleval3').innerText = jw.slice(66, nameCounter);
    } else if (nameCounter == 68 + CL) { // }
        document.getElementById('titleval3').style.color = '#bbf49c';
        document.getElementById('titlebracket2').innerText = jw.slice(67 + CL, nameCounter);
        document.getElementById('underscore4').style.display = 'none';
...
function animateCPP(CL) {
    if (nameCounter <= 5) { // class
        ...
        document.getElementById('underscore3').style.display = 'none';
        document.getElementById('underscore4').style.display = 'inline';
    } else if (nameCounter <= 69) { // \tint
        document.getElementById('titletype3').innerText = jw.slice(65, nameCounter);
    } else if (nameCounter <= 79) { // pageViews
        if (nameCounter == 70) { document.getElementById('titletype3').style.color = '#179fff' };
        document.getElementById('titlestruct3').innerText = jw.slice(69, nameCounter);
    } else if (nameCounter <= 81) { // _=
        document.getElementById('titlestruct3').style.color = '#9cdcfe';
        document.getElementById('titlecolon3').innerText = jw.slice(79, nameCounter);
    } else if (nameCounter <= 82 + CL) { // _{CLICK_COUNT}
        document.getElementById('titleval3').innerText = jw.slice(81, nameCounter);
    } else if (nameCounter == 83 + CL) {
        document.getElementById('titlecomma3').innerText = jw.slice(82 + CL, nameCounter);
    } else if (nameCounter == 84 + CL) { // }
        document.getElementById('titleval3').style.color = '#bbf49c';
        document.getElementById('titlebracket2').innerText = jw.slice(83 + CL, nameCounter);
        document.getElementById('underscore4').style.display = 'none';
        document.getElementById('underscore5').style.display = 'inline';
    } else if (nameCounter == 85 + CL) {
        document.getElementById('titlebracket2').style.color = '#ffcb05';
...
function animatePY(CL) {
    ...
    } else if (nameCounter <= 101) { // page_views
        document.getElementById('titlestruct3').innerText = jw.slice(91, nameCounter);
    } else if (nameCounter <= 103) { // _=
        document.getElementById('titlestruct3').style.color = '#9cdcfe';
        document.getElementById('titlecolon3').innerText = jw.slice(101, nameCounter);
    } else if (nameCounter <= 104 + CL) { // _{CLICKS_LENGTH}
        document.getElementById('titleval3').innerText = jw.slice(103, nameCounter);
    } else {
        document.getElementById('titleval3').style.color = '#bbf49c';
        document.getElementById('arrows').style.display = 'block';
        document.getElementById('arrows').classList.add('fadein');
...
async function startJS() {
    ...
    const response = await fetch('https://api.counterapi.dev/v2/jonathan-wylies-team-2751/jw-js-counter/up')
    const res = await response.json();
    const clicks = await res.data.up_count;
    const clicksLen = String(clicks).length;

    jw = `const jonathanWylie = {\tstudent: true,\tdeveloper: true,\tpageViews: ${clicks}}`;
    nameTimer = setInterval(animateJS, 35, clicksLen);
}
...
async function startCPP() {
    ...
    const response = await fetch('https://api.counterapi.dev/v2/jonathan-wylies-team-2751/jw-cpp-counter/up')
    const res = await response.json();
    const clicks = await res.data.up_count;
    const clicksLen = String(clicks).length;

    jw = `class jonathanWylie {\tbool student = true;\tbool developer = true;\tint pageViews = ${clicks};};`;
    nameTimer = setInterval(animateCPP, 35, clicksLen);
}
...
async function startPY() {
    ...
    const response = await fetch('https://api.counterapi.dev/v2/jonathan-wylies-team-2751/jw-python-counter/up')
    const res = await response.json();
    const clicks = await res.data.up_count;
    const clicksLen = String(clicks).length;

    document.getElementById('pytab').classList.add('selectedtab');
    jw = `class jonathanWylie:\tdef __init__(self):\t\tself.student = True\t\tself.developer = True\t\tself.page_views = ${clicks}`;
    nameTimer = setInterval(animatePY, 35, clicksLen);
}
